...
const getTodos = () => convertStringToObj(sessionStorage.getItem(storageKey));

const filterTodos = (searchStr) => getTodos().filter(todo => todo.toLowerCase().includes(searchStr.toLowerCase()));

const addTodo = (todo) => sessionStorage.setItem(storageKey, convertObjToString([...getTodos(), todo]));

const deleteTodo = (todo) => sessionStorage.setItem(storageKey, convertObjToString(getTodos().filter(_todo => _todo !== todo)));
...
const initClickListeners = () => {
    Array.from(document.getElementsByClassName("list-group-item")).forEach(_item => {
        _item.addEventListener("click", ($event) => {
            ...
            if(window.confirm("Have you completed this task: " + todo + " ? " + " Do you want to delete this task?")) {
                deleteTodo(todo);
                displayTodos();
            }
        });
